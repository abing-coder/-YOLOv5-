...
class ChannelAttention(nn.Module):
    ...
    def __init__(self, channels, ratio=16):
        """Initializes channel attention with shared MLP on avg/max pooled descriptors."""
        super().__init__()
    ...
    def forward(self, x):
        """Applies channel attention weights."""
...
class SpatialAttention(nn.Module):
    ...
    def __init__(self, kernel_size=7):
        """Initializes spatial attention with avg/max channel descriptors."""
        super().__init__()
...
class CBAM(nn.Module):
    ...
    def __init__(self, c1, ratio=16, kernel_size=7):
        """Initializes CBAM for feature refinement under noisy backgrounds."""
        super().__init__()
        self.ca = ChannelAttention(c1, ratio=ratio)
        self.sa = SpatialAttention(kernel_size=kernel_size)

    def forward(self, x):
        """Applies channel then spatial attention."""
        x = x * self.ca(x)
        x = x * self.sa(x)
        return x
...
class GhostConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, g=1, act=True):
        ...
        """
        super().__init__()
        c_ = math.ceil(c2 / 2)  # hidden channels
        self.cv1 = Conv(c1, c_, k, s, None, g, act=act)
        self.cv2 = Conv(c_, c2 - c_, 5, 1, None, c_, act=act)
